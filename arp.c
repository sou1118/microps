...
static struct arp_cache *arp_cache_select(ip_addr_t pa) {
    ...
    // Iterate through the ARP cache entries to find a match
    for (entry = caches; entry < tailof(caches); entry++) {
        // Check if the entry is in use and matches the given IP address
...
static int arp_request(struct net_iface *iface, ip_addr_t tpa) {
    ...
    // The source hardware address is the address of the interface.
    memcpy(request.sha, iface->dev->addr, ETHER_ADDR_LEN);

    // The source protocol address is the address of the interface.
    memcpy(request.spa, &((struct ip_iface *)iface)->unicast, IP_ADDR_LEN);

    // The target hardware address is zero.
    memset(request.tha, 0, ETHER_ADDR_LEN);

    // The target protocol address is the address being resolved.
...
static void arp_input(const uint8_t *data, size_t len, struct net_device *dev) {
    ...
    if (ntoh16(msg->hdr.hrd) != ARP_HRD_ETHER ||
        ...
        errorf("unsupported hardware address, address=0x%04x",
               ntoh16(msg->hdr.hrd));
        return;
    }
    if (ntoh16(msg->hdr.pro) != ARP_PRO_IP || msg->hdr.pln != IP_ADDR_LEN) {
        errorf("unsupported protocol address, address=0x%04x",
               ntoh16(msg->hdr.pro));
        return;
    }
