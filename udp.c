...
int udp_bind(int id, struct ip_endpoint *local) {
    ...
    if (exist) {
        mutex_unlock(&mutex);
        errorf("already bound: id=%d, want=%s, exist=%s", id,
...
ssize_t udp_sendto(int id, uint8_t *data, size_t len,
    ...
    if (local.addr == IP_ADDR_ANY) {
        ...
        if (!iface) {
            errorf("iface is not found that can reach foreign address, addr=%s",
    ...
    if (!pcb->local.port) {
        ...
        if (!pcb->local.port) {
            debugf("failed to dynamic assign locak port, addr=%s",
                   ip_addr_ntop(local.addr, addr, sizeof(addr)));
            mutex_unlock(&mutex);
            return -1;
        }
