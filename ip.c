...
static struct ip_route *ip_route_add(ip_addr_t network, ip_addr_t netmask,
    ...
    route->next = routes;
    routes = route;
    infof("route added: network=%s, netmask=%s, nethop=%s, iface=%s, dev=%s",
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
    ...
    if (hdr->vhl >> 4 != IP_VERSION_IPV4) {
        errorf("ip version error: v=%u", hdr->vhl >> 4);
    }
    hlen = (hdr->vhl & 0x0f) << 2;
    if (len < hlen) {
    ...
    debugf("dev=%s, iface=%s, total=%u", dev->name,
           ip_addr_ntop(iface->unicast, addr, sizeof(addr)), total);
    ip_dump(data, total);
...
static ssize_t ip_output_core(struct ip_iface *iface, uint8_t protocol,
    ...
    debugf("dev=%s, dst=%s, protocol=%u, len=%u", NET_IFACE(iface)->dev->name,
           ip_addr_ntop(dst, addr, sizeof(addr)), protocol, hlen + len);
